...
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php 
        include "koneksi.php";
        $result=mysqli_query($conn,"select * from peminjamanbuku order by id_peminjaman_buku desc");
        $no=0;
        while($dt_histori=mysqli_fetch_array($result)){
            $no++;
            //menampilkan buku yang dipinjam
            $buku_dipinjam="<ol>";
            $qry_buku=mysqli_query($conn,"select * from  detail_peminjaman_buku join tabelbuku on tabelbuku.id_buku=detail_peminjaman_buku.id_buku where id_peminjaman_buku = '".$dt_histori['id_peminjaman_buku']."'");
            while($dt_buku=mysqli_fetch_array($qry_buku)){
                $buku_dipinjam.="<li>".$dt_buku['judul_buku']."</li>";
            }
            $buku_dipinjam.="</ol>";
            //menampilkan status sudah kembali atau belum
            $qry_cek_kembali=mysqli_query($conn,"select * from pengembalianbuku where id_peminjaman_buku = '".$dt_histori['id_peminjaman_buku']."'");
